...
<head>
    ...
    <title>Hospital Universitario - {{ $especialidad }}</title>
    <!-- Bootstrap 5 -->
    ...
    <div class="row flex-nowrap">
        ...
        <div class="col py-3">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4 bg-white p-3 rounded shadow-sm">
                <div class="w-50">
                    <input type="text" class="form-control rounded-pill" placeholder="Buscar médico o legajo...">
                </div>
            ...
            <div class="card border-0 shadow-sm">
                <div class="card-header bg-white py-3">
                    <h5 class="fw-bold m-0 text-dark">Tabla de médicos</h5>
                </div>
                        ...
                        <tbody>
                            @forelse($doctors as $doctor)
                                <tr>
                                    ...
                                    <td>MED-{{ str_pad($doctor->id, 3, '0', STR_PAD_LEFT) }}</td>
                                    <td>HOSP-DOC-{{ $doctor->hospital_code ?? '421' }}</td>
                                    <td><strong>{{ $doctor->studies_count }}</strong></td>
                                    <td class="text-center">
                                        <a href="{{ route('rrhh.doctor.detail', $doctor->id) }}" class="btn btn-primary btn-sm px-3 fw-bold">
                                            Ver
                                        </a>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="5" class="text-center py-4 text-muted">No hay médicos registrados en esta especialidad.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
